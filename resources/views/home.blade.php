        ...
        @foreach($posts as $post)
                <div class="post">
                        ...
                        @if(Auth::user()->id == $post->user_id)
                            <div class="head-button">
                                ...
                                <div class="optiones">
                                    ...
                                    {!! Form::open(['action' => ['PostsController@destroy', $post->id], 'method' => 'POST', 'id' => 'detele-form' . $post->id]) !!}
                                        <button type="submit" form="detele-form{{$post->id}}" class="delete-btn"><i class="fas fa-trash-alt"></i> Delete</button>
                    ...
                    <div class="post-details">
                        ...
                        {!! Form::open(['action' => ['PostsController@update', $post->id ], 'method' => 'POST', 'enctype' => 'multipart/form-data', 'class' => 'edit-form', 'id' => 'edit-form' . $post->id]) !!}
                ...
                <div class="comment-section">
                    <h3>Add a Comment</h3>
                    {!! Form::open(['action' => 'CommentsController@store', 'method' => 'POST', 'enctype' => 'multipart/form-data', 'id' => 'comment-form' . $post->id]) !!}
                        {!! Form::textarea('body', '', ['placeholder' => 'Type your comment...']) !!}
                        {!! Form::hidden('post-id', $post->id)!!}
                        <button form="comment-form{{$post->id}}" type="submit" class='comment-btn'><i class="far fa-comment-dots"></i></button>
                    {!! Form::close() !!}
                    <section class="fix"></section>
                </div>
